...
export function BoostBusinessModal({
  ...
  const [adminBarcodeUrl, setAdminBarcodeUrl] = useState<string>(() => localStorage.getItem('vyapar_barcode_url') || '');
  const [adminUpiId, setAdminUpiId] = useState<string>(() => localStorage.getItem('vyapar_admin_upi_id') || 'ashish660@ibl');
  ...
  // Fetch live Admin Barcode and UPI from Firestore
  useEffect(() => {
    if (!isOpen) return;
    const cached = localStorage.getItem('vyapar_barcode_url');
    if (cached) setAdminBarcodeUrl(cached);

    fetch('/api/settings/admin')
      .then(res => res.json())
      .then(data => {
        if (data?.barcodeImageUrl) {
          setAdminBarcodeUrl(data.barcodeImageUrl);
          localStorage.setItem('vyapar_barcode_url', data.barcodeImageUrl);
        }
        if (data?.upiId) {
          setAdminUpiId(data.upiId);
          localStorage.setItem('vyapar_admin_upi_id', data.upiId);
        }
      })
      .catch(() => {});
  }, [isOpen]);
              ...
              <AnimatePresence mode="wait" custom={slideDirection}>
                ...
                {currentStep === 4 && (
                  ...
                  >
                    ...
                    {/* Official Barcode QR Code Display */}
                    <div className="p-3.5 rounded-2xl bg-white dark:bg-zinc-900 border-2 border-amber-400/60 dark:border-zinc-800 flex flex-col items-center text-center shadow-md">
                      <span className="text-[10px] font-black uppercase tracking-wider text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/60 px-2.5 py-0.5 rounded-full border border-amber-300 dark:border-amber-700 mb-2">
                        📷 Scan Admin QR Code to Pay ₹{selectedPlan === 'yearly' ? '1,188' : '99'}
                      </span>

                      <div className="w-48 h-48 sm:w-52 sm:h-52 bg-white rounded-2xl p-2.5 border-2 border-dashed border-amber-400 shadow-sm relative overflow-hidden flex items-center justify-center">
                        {adminBarcodeUrl ? (
                          <img 
                            src={adminBarcodeUrl} 
                          ...
                          <img 
                            src={`https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(`upi://pay?pa=${adminUpiId}&pn=VyaparBridge&am=${selectedPlan === 'yearly' ? '1188' : '99'}&cu=INR`)}`}
                            alt="UPI QR Code" 
                            className="w-full h-full object-contain rounded-xl"
                          />
                        )}
                      ...
                      <p className="text-[11px] font-extrabold text-slate-700 dark:text-zinc-300 mt-2">
                        Scan with GPay, PhonePe, Paytm, BHIM, or any UPI App
                      </p>
                      
                      <div className="flex items-center justify-center gap-2 mt-1.5 w-full">
                        <span className="text-xs font-mono font-bold text-slate-900 dark:text-white bg-slate-100 dark:bg-zinc-800 px-3 py-1 rounded-lg border border-slate-200 dark:border-zinc-700">
                          {adminUpiId}
                        </span>
                        <button 
                          type="button"
                          onClick={() => {
                            ...
                            toast.success(`📋 UPI ID Copied: ${adminUpiId}`);
                          }}
                          className="px-2.5 py-1 rounded-lg bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs flex items-center gap-1 shadow-2xs cursor-pointer"
                        >
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 space-y-2.5">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">
                          Or Click to Pay directly in App:
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <button
                          ...
                          className="py-2.5 px-2 rounded-xl bg-purple-700 hover:bg-purple-800 text-white font-extrabold text-xs flex items-center justify-center gap-1 shadow-sm transition-all cursor-pointer"
                        >
                          <span>PhonePe</span>
                        </button>
                        ...
                        <button
                          ...
                          className="py-2.5 px-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs flex items-center justify-center gap-1 shadow-sm transition-all cursor-pointer"
                        >
                          <span>GPay</span>
                        </button>
                        ...
                        <button
                          ...
                          className="py-2.5 px-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-extrabold text-xs flex items-center justify-center gap-1 shadow-sm transition-all cursor-pointer"
                        >
                          <span>Paytm</span>
                        </button>
                      </div>
