...
export const WelcomeSplash: React.FC<WelcomeSplashProps> = ({ onComplete }) => {
  const isAlreadySeen = typeof window !== 'undefined' && localStorage.getItem('vyapar_splash_shown') === '1';
  const [isVisible, setIsVisible] = useState(!isAlreadySeen);
  ...
  const handleFinish = () => {
    ...
    try {
      localStorage.setItem('vyapar_splash_shown', '1');
    } catch(e) {}
    setIsFadingOut(true);
    setTimeout(() => {
      setIsVisible(false);
  ...
  // Ultra-Fast 0.8s Brand Welcome Progress Bar (Never blocks user)
  useEffect(() => {
    if (isAlreadySeen) {
      if (onComplete) onComplete();
      return;
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        const next = prev + 5;
        if (next < 30) {
          setStatusText("Connecting Verified Trade Hubs...");
        } else if (next < 70) {
          setStatusText("Loading Direct B2B Commerce...");
        } else if (next < 95) {
  ...
  if (!isVisible || isAlreadySeen) return null;
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-[99999] flex flex-col items-center justify-between bg-black text-white transition-all duration-300 select-none overflow-hidden cursor-pointer ${
        isFadingOut ? "opacity-0 pointer-events-none scale-105" : "opacity-100 scale-100"
      }`}
    >
      {/* Top Header */}
      <header className="w-full max-w-5xl mx-auto px-4 sm:px-6 pt-4 sm:pt-6 pb-2 flex items-center justify-between z-30">
        {/* Brand Logo & Name */}
        ...
        <button
          ...
          className="px-3 py-1 bg-white/10 hover:bg-white/20 border border-white/20 rounded-full text-xs font-bold text-white transition-colors"
        >
          Skip ✕
        </button>
      </header>
