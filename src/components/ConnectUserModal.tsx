...
export function ConnectUserModal({ targetUser, onClose, currentUser }: ConnectUserModalProps) {
  ...
  const isSellerUser = currentUser && currentUser.role !== 'customer' && currentUser.role !== 'buyer';
  const isCustomerUser = !currentUser || currentUser.role === 'customer' || currentUser.role === 'buyer';
  
  const isGolden1188 = isSellerUser && (
    currentUser?.verifiedPlan === '1_year_pro' ||
    currentUser?.verifiedPlan === 'yearly' ||
    currentUser?.subscriptionPlan === '1_year_pro' ||
    currentUser?.subscriptionPlan === 'yearly' ||
    currentUser?.goldenBadge ||
    (typeof currentUser?.subscriptionAmount === 'number' && currentUser.subscriptionAmount >= 1188) ||
    currentUser?.activeFreeOneYearPlan
  );

  const isVerified99Buyer = isCustomerUser && (
    currentUser?.isVerified ||
    currentUser?.verifiedPlan === '99' ||
    currentUser?.membershipType ||
    currentUser?.subscriptionPlan
  );

  const checkContactPermissions = (): boolean => {
    if (isCustomerUser && !isVerified99Buyer) {
      alert("🔒 Seller se direct WhatsApp/Call/Inquiry contact karne ke liye ₹99 Verified Buyer Plan (Red Badge) lena zaroori hai.");
      window.dispatchEvent(new CustomEvent('openVerifyModal', { detail: { role: 'customer' } }));
      onClose();
      return false;
    }

    if (isSellerUser && !isGolden1188) {
      alert("🔒 Customer ya Direct B2B Contact ke liye Seller ka ₹1,188/Year Golden Plan se verified hona zaroori hai.");
      window.dispatchEvent(new CustomEvent('openVerifyModal', { detail: { role: currentUser?.role || 'dealer' } }));
      onClose();
      return false;
    }
  ...
  const handleStartInAppChat = () => {
    if (!checkContactPermissions()) return;
    onClose();
  ...
  const handleViewProfile = () => {
    onClose();
    navigate(`/profile/${encodeURIComponent(targetUser.id || targetUser.name || targetUser.username)}`);
