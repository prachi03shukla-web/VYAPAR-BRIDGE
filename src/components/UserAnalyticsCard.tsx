...
import { Eye, Heart, MessageCircle, Share2, Bookmark, BarChart2, TrendingUp, Sparkles, X, Users, Clock, Filter, Star } from 'lucide-react';
...
export function UserAnalyticsCard({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    // Auto-refresh every 2 seconds for real-time stats updates
    ...
    <div className="mb-5 bg-gradient-to-r from-slate-900 via-zinc-900 to-slate-950 p-4 rounded-2xl border border-blue-900/50 text-white shadow-xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-2">
        ...
        <div className="bg-zinc-800/80 border border-zinc-700/60 p-2 rounded-xl flex items-center gap-2">
          ...
          <div>
            <div className="text-[10px] text-zinc-400 font-medium font-bold text-amber-400">Reputation</div>
            <div className="text-xs font-black text-white">{summary.ratingAverage ? Number(summary.ratingAverage).toFixed(1) : '0.0'} ({summary.ratingCount || 0})</div>
          </div>
        </div>
      </div>
