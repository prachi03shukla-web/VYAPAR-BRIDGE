...
export function generateInstagramFeed(
  ...
  const candidatePosts = db.posts.filter(p => {
    ...
    // Filter out posts with expired ephemeral blob: URLs or test descriptions
    let media = String(p.mediaUrl || p.thumbnailUrl || '');
    if (media.startsWith('blob:')) {
      if (p.persistentMediaUrl && !p.persistentMediaUrl.startsWith('blob:')) {
        p.mediaUrl = p.persistentMediaUrl;
        media = p.persistentMediaUrl;
